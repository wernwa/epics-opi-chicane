...
class TabStripChartVolt(wx.Panel):
    ...
    def create_main_panel(self):
        ...
        self.ListNames_to_magn = {
            q1_volt.pvname : mquad1,
            q2_volt.pvname : mquad2,
            q3_volt.pvname : mquad3,
            q4_volt.pvname : mquad4,
            q5_volt.pvname : mquad5,
            q6_volt.pvname : mquad6,
            q7_volt.pvname : mquad7,
            d1_volt.pvname : mdipol1,
            d2_volt.pvname : mdipol2,
        }
    ...
    def init_plot(self):
        ...
        self.axes.set_title('Voltage', size=12)
    ...
    def draw_plot(self):
        ...
        if self.xmax_control.is_auto():
            ...
            xmax = mquad1.strip_chart_volt_time[-1] if mquad1.strip_chart_volt_time[-1] > 50 else 50
        else:
        ...
        if self.ymin_control.is_auto():
            ...
            ymin = mquad1.strip_chart_volt[-1] - 50
        else:
            ymin = int(self.ymin_control.manual_value())

        if self.ymax_control.is_auto():
            #ymax = round(max(mquad1.strip_chart_temp), 0) + 1
            ymax = mquad1.strip_chart_volt[-1]
        else:
    ...
    def on_update_axes(self):
        while self.alive:
            while self.update_axes:
                ...
                self.axes.set_ylabel('Voltage in Volt', fontsize = 9)
                ...
                for pv_i in self.lb.GetChecked():
                    ...
                    xmin_i = 0
                    for i in xrange(0,len(magn.strip_chart_volt_time)):
                        if self.xmin<=magn.strip_chart_volt_time[i]:
                            xmin_i = i
                            break
                    xmax_i = len(magn.strip_chart_volt_time)-1
                    for i in xrange(len(magn.strip_chart_volt_time)-1,0,-1):
                        if self.xmax==magn.strip_chart_volt_time[i]:
                            xmax_i = i
                            break


                    if xmax_i-xmin_i < self.max_datapoints_plot:
                        part_arr_x = magn.strip_chart_volt_time[xmin_i:xmax_i]
                        part_arr_y = magn.strip_chart_volt[xmin_i:xmax_i]
                    else:
                        ...
                        for i in range(xmin_i,xmax_i+1,step):
                            #ii = int(round(i*step))
                            x_arr.append(magn.strip_chart_volt_time[i])
                            y_arr.append(magn.strip_chart_volt[i])
                        if l%step!=0:
                            x_arr.append(magn.strip_chart_volt_time[-1])
                            y_arr.append(magn.strip_chart_volt[-1])
                        part_arr_x = x_arr
                        part_arr_y = y_arr
    ...
    def on_redraw_timer(self, event):
        ...
        if not self.paused and self.parent.GetPageText(tab_i)=='Voltage':
            self.draw_plot()
